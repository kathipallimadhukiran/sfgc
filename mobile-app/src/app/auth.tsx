...
export default function AuthScreen() {
  ...
  const handleRegister = async () => {
    ...
    if (password !== confirmPassword) {
      setError(isTel ? 'పాస్‌వర్డ్‌లు సరిపోలడం లేదు.' : 'Passwords do not match.');
      return;
    }
    ...
    try {
      ...
      setError(err.message || (isTel ? 'రిజిస్ట్రేషన్ విఫలమైంది.' : 'Registration failed.'));
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <View style={[styles.headerBar, { borderBottomColor: theme.cardBorder }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.text }]}>
          {authTab === 'login' 
            ? (isTel ? 'సైన్ ఇన్ (Sign In)' : 'Sign In') 
            : (isTel ? 'సభ్యుల నమోదు (Register)' : 'Member Registration')}
        </Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      >
        ...
        <View style={[styles.tabSelector, { backgroundColor: isDark ? '#1e293b' : '#f1f5f9' }]}>
          <TouchableOpacity
            style={[
              styles.tabButton,
              authTab === 'login' && { backgroundColor: theme.primary, elevation: 2 }
            ]}
          ...
          >
            <MaterialCommunityIcons 
              name="login" 
              size={18} 
              color={authTab === 'login' ? '#ffffff' : (isDark ? '#94a3b8' : '#64748b')} 
              style={{ marginRight: 6 }}
            />
            <Text style={[
          ...
          <TouchableOpacity
            style={[
              styles.tabButton,
              authTab === 'register' && { backgroundColor: theme.primary, elevation: 2 }
            ]}
            onPress={() => { setAuthTab('register'); setError(''); }}
            activeOpacity={0.8}
          >
            <MaterialCommunityIcons 
              name="account-plus-outline" 
              size={18} 
              color={authTab === 'register' ? '#ffffff' : (isDark ? '#94a3b8' : '#64748b')} 
              style={{ marginRight: 6 }}
            />
            <Text style={[
              styles.tabButtonText,
        ...
        {/* SIGN IN FORM */}
        {authTab === 'login' ? (
          <Card style={[styles.card, { backgroundColor: theme.backgroundElement, borderColor: theme.cardBorder }]}>
            <Card.Content>
              ...
              <Text style={[styles.inputLabel, { color: theme.text, marginTop: 14 }]}>{isTel ? 'పాస్‌వర్డ్ *' : 'Password *'}</Text>
              <TextInput
                mode="outlined"
                value={loginPassword}
          ...
          <Card style={[styles.card, { backgroundColor: theme.backgroundElement, borderColor: theme.cardBorder }]}>
            <Card.Content>
              ...
              <Text style={[styles.formSubHeader, { color: theme.textSecondary }]}>
                {isTel ? 'సంఘంలో భాగస్వామ్యం పొందడానికి మీ వివరాలను నమోదు చేయండి.' : 'Create your church account to connect with prayer teams and ministries.'}
              </Text>

              <Text style={[styles.inputLabel, { color: theme.text }]}>{isTel ? 'పూర్తి పేరు *' : 'Full Name *'}</Text>
              <TextInput
                mode="outlined"
                value={regData.name}
              ...
              <Text style={[styles.inputLabel, { color: theme.text, marginTop: 12 }]}>{isTel ? 'పాస్‌వర్డ్ *' : 'Password *'}</Text>
              <TextInput
                mode="outlined"
              ...
              <Text style={[styles.inputLabel, { color: theme.text, marginTop: 12 }]}>{isTel ? 'పాస్‌వర్డ్ నిర్ధారణ *' : 'Confirm Password *'}</Text>
              <TextInput
                mode="outlined"
              ...
              <Text style={[styles.inputLabel, { color: theme.text, marginTop: 12 }]}>{isTel ? 'నివాస ప్రాంతము / బ్రాంచ్' : 'Location / Branch'}</Text>
              <TextInput
                mode="outlined"
              ...
              <View style={{ flexDirection: 'row', gap: 10, marginTop: 12 }}>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.inputLabel, { color: theme.text }]}>{isTel ? 'పుట్టినరోజు' : 'Birthday'}</Text>
                  <TouchableOpacity onPress={() => handleOpenDatePicker('birthday')} activeOpacity={0.8}>
                    <View pointerEvents="none">
                      <TextInput
                        mode="outlined"
                ...
                <View style={{ flex: 1 }}>
                  <Text style={[styles.inputLabel, { color: theme.text }]}>{isTel ? 'బాప్తిస్మ తేదీ' : 'Baptism Date'}</Text>
                  <TouchableOpacity onPress={() => handleOpenDatePicker('baptismDate')} activeOpacity={0.8}>
                    <View pointerEvents="none">
                      <TextInput
                        mode="outlined"
              ...
              >
                {isTel ? 'ఖాతా సృష్టించండి (Register)' : 'Create Church Account'}
              </Button>
